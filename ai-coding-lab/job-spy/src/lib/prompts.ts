...
你的任务是将候选人的简历信息与岗位要求进行逐项对比，并给出一个整体匹配分数。

## 输出要求（必须严格遵守）

你必须输出以下 JSON 结构（字段名必须是英文）：

{
  "overallMatchScore": <number 0-100>,   // ← 这是最重要的字段！根据下面的评分规则计算
  "matchedSkills": ["完全匹配的技能名称"],
  "partialMatches": [
    { "jdSkill": "JD要求的技能", "userSkill": "候选人相关技能", "reason": "匹配理由" }
  ],
  "missingSkills": ["候选人缺失的技能"],
  "extraSkills": ["候选人额外拥有的技能"]
}

## 匹配规则

1. **完全匹配**：候选人的技能与 JD 要求完全一致或高度吻合 → 纳入 matchedSkills
2. **部分匹配**：候选人有相关但不完全对口的技能（例如 JD 要求 React，候选人会 Vue.js —— 都是前端框架，算部分匹配）→ 纳入 partialMatches
3. **缺失**：候选人完全没有相关经验 → 纳入 missingSkills
4. **额外技能**：候选人拥有但 JD 未提及的技能 → 纳入 extraSkills

## overallMatchScore 评分规则
...
- overallMatchScore 必须是数字类型（number），不是字符串
- 做语义匹配，而不仅仅是关键词匹配
...
4. **长度控制**：整体内容必须恰好填满 2 页 A4 纸（约 55-70 行内容，10.5pt 字号），不能溢出到第 3 页，也不能在第 1 页留大片空白。

## ⭐ 篇幅分配策略（重要）

- **项目经历（重点展开）**：每个项目写 4-6 条 highlights，详细描述技术方案、架构决策、量化成果、业务价值。description 写 2 句话概括项目背景和规模。这部分应占总篇幅的 50% 以上。
- **工作经历（简要概括）**：每段工作写 2-3 条 highlights，只保留最核心的职责和关键成果。避免与项目经历重复。这部分占 20-25%。
- **专业技能**：分 4-6 个类别，每个类别列 3-8 项具体技能。占 10-15%。
- **职业摘要**：4-6 句话，充分展开核心竞争力和行业洞察。占 5-10%。
- **教育背景 + 证书**：简洁列出即可。
...

{
  ...
  "professionalSummary": "AI 提炼的职业摘要，4-6 句话深度概括核心竞争力、技术栈深度、行业经验、代表性成就，直接对标 JD 亮点",
  "skills": [
    {
      "category": "技能分类名（如「前端开发」「后端开发」「数据库」「工具与平台」「测试与质量」「领域知识」）",
      "items": ["技能1", "技能2", ...]
    }
  ],
  "workExperience": [
    {
      ...
      "highlights": ["简要工作成果1（2-3条即可）", "简要工作成果2"]
    }
  ],
  "projectExperience": [
    {
      "name": "项目名",
      "role": "担任角色",
      "description": "项目简述（2-3句话，含项目规模、技术栈、业务目标）",
      "highlights": ["详细项目成果1（4-6条，每条具体描述面临的挑战、技术方案、量化结果）", "详细项目成果2", ...]
    }
  ],
  "education": [
    {
...
1. **项目经历详细展开**：每个项目写 4-6 条 highlights，详细描述技术方案、架构决策、量化成果。description 写 2 句背景介绍。如果原始简历中项目细节不足，请从经历挖掘对话中提取补充。
2. **工作经历简要概括**：每段工作只写 2-3 条 highlights，抓最核心的职责和成果，不与项目经历重复。
3. **职业摘要充分展开**：写 4-6 句话，深度刻画候选人的核心竞争力。
4. **技能分 4-6 个类别**，每个类别 3-8 项具体技能，尽可能全面。
5. **总篇幅必须恰好填满 2 页 A4 纸**（约 55-70 行内容），不可溢出到第 3 页，不可在第 1 页留大片空白。
